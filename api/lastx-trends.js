...
function trends(stats, teamName) {
  ...
  if (over85Corners >= 3) out.push({ tipo: 'great', texto: `O mercado de escanteios também chama atenção: Mais de 8.5 escanteios ocorreu em ${over85Corners} dos últimos ${jogos} jogos.` });
...
export default async function handler(req, res) {
  try {
    ...
    const homeName = req.query.home_name || req.query.home || 'Mandante';
    const awayName = req.query.away_name || req.query.away || 'Visitante';

    if (!seasonId || !homeId || !awayId) {
      return res.status(400).json({
        ok: false,
        error: 'season_id, home_id e away_id são obrigatórios.',
        received: { seasonId, homeId, awayId }
      });
    }
    ...
    const payload = { ok: true, season_id: Number(seasonId), home, away };
