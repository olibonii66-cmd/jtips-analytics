...
function parseCSV(text) {
  ...
  const data = rows.slice(1).map(cells => {
    const obj = {};
    columns.forEach((h, index) => {
      obj[h] = cells[index] !== undefined ? String(cells[index]).trim() : '';
    });
    return obj;
  });
...
async function fetchLeague({ fonte, data, league, token }) {
  const url = new URL(`${API_BASE}/${fonte}/download/`);
  ...
  if (!response.ok) {
    return { ok: false, league, status: response.status, error: text.slice(0, 500), rows: [], columns: [] };
  }
...
export default async function handler(req, res) {
  try {
    ...
    if (!ALLOWED_SOURCES.includes(fonte)) {
      return res.status(400).json({ ok: false, error: 'Fonte inválida.', fontes_liberadas: ALLOWED_SOURCES });
    }
    ...
    if (invalid.length) {
      return res.status(403).json({
        ok: false,
        error: 'Uma ou mais ligas não estão liberadas.',
        ligas_invalidas: invalid,
        ligas_liberadas: ALLOWED_LEAGUES,
      });
    }

    const results = await Promise.all(leagues.map(league => fetchLeague({ fonte, data, league, token })));
    ...
    return res.status(200).json({
      ...
      league: leagueParam,
      ligas_liberadas: ALLOWED_LEAGUES,
      ligas_consultadas: leagues,
      ligas_com_dados: results.filter(r => r.rows.length > 0).map(r => r.league),
      total: rows.length,
      count: rows.length,
      columns,
      dados: rows,
